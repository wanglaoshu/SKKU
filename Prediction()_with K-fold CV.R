...
data_r <- read.csv("/Users/heeyoungpark/Desktop/연구실 stuff/final_dataset_csv.csv", header=TRUE, sep=',')
class(data)
data <- data[,c("SHIP_NO",'RSN_CD','DWG_TYPE','DWG_BLOCK','DWG_PROC','DWG_STAGE')]
data$SHIP_NO <- substr(data$SHIP_NO,1,1)
data_r$SHIP_NO <- substr(data_r$SHIP_NO,1,1)
data$SHIP_NO <- sapply(data$SHIP_NO, as.factor)
data_r$SHIP_NO <- sapply(data_r$SHIP_NO, as.factor)
data <- as(data,'transactions')
RSN <- grep("^RSN_CD=",itemLabels(data), value = TRUE)
train_pct <- 0.7
nrow(data)*train_pct
data_test <- data[as.integer(nrow(data)*train_pct):nrow(data),]
data_test_r <- data_r[as.integer(nrow(data_r)*train_pct):nrow(data_r),]
rules <- apriori(data = data[1:nrow(data)*train_pct,], parameter = list(support = 0.01, confidence = 0.4), 
                 appearance = list(rhs = RSN))
rules_full <- apriori(data = data, parameter = list(support = 0.01, confidence = 0.4), 
                      appearance = list(rhs = RSN))
...
library(tidyr)
rulesSaved <-write.csv(file='rules_.csv', x=rules_df, row.names=TRUE)
rules_df <- as(rules,'data.frame')
rules_df <- separate(data = rules_df, col = rules, into = c("Input",'Output'), sep = '=>')
write.csv(file='rules_df2.csv', x=rules_df, row.names = TRUE)
...
prediction(data = data, Support = 0.001, Confidence = 0.4,'SHIP_NO=2','DWG_BLOCK=S1','DWG_TYPE=S','DWG_PROC=C','DWG_STAGE=1')
